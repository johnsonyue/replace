...
#target retrieving.
#latest time.
def get_latest_time_fromsite(username, password, reverse=False): #reverse is used to get the earliest time.
	...
	for t in team_dir:
		...
		temp.append(parse_latest_year(url+t+e, opener, reversed))
	
	res = temp[0]
	for t in temp[1:]:
		if(not reversed and time_cmp(t, res) > 0):
			res = t
		if(reversed and time_cmp(t, res) < 0):
			res = t
